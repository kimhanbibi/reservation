import streamlit as stimport pandas as pdimport calendarfrom datetime import date, datetimefrom pathlib import Path
...
st.set_page_config(page_title="실험 예약 시스템",page_icon="🧪",layout="wide")
...
BASE_DIR = Path(file).resolve().parentDATA_FILE = BASE_DIR / "reservations.csv"

GROUPS = [f"{i}조" for i in range(1, 11)]AVAILABLE_TIMES = ["09:00-10:00", "10:00-11:00", "11:00-12:00", "12:00-13:00","13:00-14:00", "14:00-15:00", "15:00-16:00", "16:00-17:00","17:00-18:00", "18:00-19:00", "19:00-20:00", "20:00-21:00","21:00-22:00",]YEAR = 2026MIN_DATE = date(YEAR, 5, 1)MAX_DATE = date(YEAR, 6, 30)MONTHS = [5, 6]WEEKDAYS = ["월", "화", "수", "목", "금", "토", "일"]
...
st.markdown(""".calendar-title {font-size: 28px;font-weight: 800;text-align: center;margin: 10px 0 20px 0;}.weekday-box {text-align: center;font-weight: 700;padding: 10px 0;background-color: #222832;border-radius: 10px;margin-bottom: 6px;}div[data-testid="stButton"] > button {width: 100%;height: 70px;border-radius: 14px;font-size: 18px;font-weight: 700;}.legend {padding: 12px;border-radius: 12px;background-color: #1f2937;margin-bottom: 14px;}""",unsafe_allow_html=True,)
...
def load_reservations():columns = ["예약ID", "예약시간", "조", "조장명", "예약날짜", "실험시간", "공지사항"]
...
def save_reservations(group, leader_name, reservation_date, reservation_times, notice):df = load_reservations()
...
def delete_reservation_ids(reservation_ids):df = load_reservations()reservation_ids = [int(x) for x in reservation_ids]df = df[~df["예약ID"].astype(int).isin(reservation_ids)]df.to_csv(DATA_FILE, index=False, encoding="utf-8-sig")

def replace_reservation_group(old_reservation_ids, group, leader_name, new_date, new_times, notice):"""선택한 예약 묶음을 삭제하고, 새 날짜/새 시간 묶음으로 다시 저장합니다."""delete_reservation_ids(old_reservation_ids)save_reservations(group, leader_name, new_date, new_times, notice)

def get_reserved_dates():df = load_reservations()if df.empty:return set()
...
if "selected_date" not in st.session_state:st.session_state.selected_date = Noneif "selected_month" not in st.session_state:st.session_state.selected_month = 5
...
with title_col:st.title("🧪 실험 예약 시스템")st.caption("달력에서 날짜를 누르고, 시간을 선택한 뒤 예약하면 해당 날짜가 파란색으로 표시됩니다.")

with notice_col:st.markdown("### 📢 전체 공지사항")st.info("""전체공지사항
...
st.divider()st.subheader("1. 조장 정보")

group = st.selectbox("본인 조", GROUPS)leader_name = st.text_input("조장 이름", placeholder="예: 홍길동")

if leader_name.strip():st.success(f"{group} 조장 {leader_name}님")
...
st.divider()left, right = st.columns([1, 1])

with left:st.subheader("2. 날짜 선택")
...
with right:st.subheader("3. 시간 선택")
...
st.divider()st.subheader("✏️ 내 예약 변경 또는 취소")

reservations = load_reservations()

if reservations.empty:st.info("아직 변경하거나 취소할 예약이 없습니다.")else:edit_col1, edit_col2 = st.columns(2)with edit_col1:edit_group = st.selectbox("변경할 조 선택", GROUPS, key="edit_group")with edit_col2:edit_name = st.text_input("조장 이름 확인", placeholder="예약할 때 입력한 이름", key="edit_name")
...
st.divider()st.subheader("📋 실험 예약 현황표")

reservations = load_reservations()

if reservations.empty:st.info("아직 예약된 실험 일정이 없습니다.")

else:reservations = reservations.sort_values(by=["예약날짜", "실험시간", "조"],ascending=True,).reset_index(drop=True)
