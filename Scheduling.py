...
# Google Sheets 설정
# =============================
SHEET_NAME = "experiment_reservations"
...
def load_reservations():
    ...
    df["예약ID"] = pd.to_numeric(df["예약ID"], errors="coerce").fillna(0).astype(int)

    return df


def save_all_reservations(df):
    worksheet = connect_sheet()
...
with title_col:
    st.title("🧪 실험 예약 시스템")
    st.caption("날짜와 시간을 선택한 뒤 예약하면 Google Sheets에 안전하게 저장됩니다.")
...
if reservations.empty:
    ...
    if edit_name.strip() and my_reservations.empty:
        ...
        my_reservations = my_reservations.sort_values(
            by=["예약날짜", "실험시간"]
        )
        ...
        if edit_mode == "날짜 단위로 한 번에 변경/취소":
            date_options = sorted(
                my_reservations["예약날짜"].astype(str).unique().tolist()
            )
...
if reservations.empty:
    ...
    board["예약자"] = board["조"].astype(str) + " / " + board["조장명"].astype(str)
    board["예약일시"] = board["예약날짜"].astype(str) + " / " + board["실험시간"].astype(str)
    board["공지사항"] = (
        board["공지사항"]
        .fillna("")
        .astype(str)
