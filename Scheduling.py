...
BACKUP_FILE = BASE_DIR / "reservations_backup.csv"
...
# 데이터 함수
# =============================
def empty_reservations_df():
    return pd.DataFrame(columns=COLUMNS)


def write_reservations(df):
    """CSV 저장. 공지사항 줄바꿈 때문에 CSV가 깨지지 않도록 모든 필드를 따옴표 처리합니다."""
    df = df.copy()
...
def load_reservations():
    if not DATA_FILE.exists():
        df = empty_reservations_df()
        write_reservations(df)
        return df
    ...
    if "예약ID" not in df.columns or df["예약ID"].astype(str).str.strip().eq("").all():
        df["예약ID"] = range(1, len(df) + 1)

    df["예약ID"] = pd.to_numeric(df["예약ID"], errors="coerce")
    df["예약ID"] = df["예약ID"].fillna(0).astype(int)

    # ID가 0인 행은 새 번호 부여
    max_id = int(df["예약ID"].max()) if not df.empty else 0
    for idx in df.index[df["예약ID"] == 0]:
        max_id += 1
        df.at[idx, "예약ID"] = max_id
...
def get_reserved_dates():
    ...
    return set(df["예약날짜"].dropna().astype(str).unique().tolist())
...
if "selected_date" not in st.session_state:
    st.session_state.selected_date = date(YEAR, 5, 1)
...
with title_col:
    st.title("🧪 실험 예약 시스템")
    st.caption("날짜와 시간을 선택한 뒤 예약하면 예약 현황표에 표시됩니다.")
...
with notice_col:
    ...
    st.info(
        ...
        **전체공지사항**
        ...
        - 실험장소는 116 강의실입니다
        - 5월 21~23일 철도학회 일정으로 실험 불가합니다
        - 매주 금요일 116에서 대학원 수업이 9:00~12:00까지 진행되니 금요일에 하실 학생은 오후에 이용 바랍니다
        - 실험종료일은 6/4일입니다
        - 궁금한 사항은 조교 김한비 또는 128강의실로 오세요
        """
    )
...
if leader_name.strip():
    st.success(f"{group} 조장 {leader_name.strip()}님")
...
with left:
    ...
    last_day = 31 if selected_month == 5 else 30
...
if reservations.empty:
    ...
    my_reservations = reservations[
        (reservations["조"].astype(str) == edit_group)
    ...
    if edit_name.strip() and my_reservations.empty:
        ...
        if edit_mode == "날짜 단위로 한 번에 변경/취소":
            ...
            selected_old_date_str = st.selectbox(
                "변경/취소할 기존 예약 날짜",
                date_options,
            )
            ...
            old_date_index = (
                change_date_options.index(old_date)
                if old_date in change_date_options
                else 0
            )
            ...
            selected_row = my_reservations[
                my_reservations["예약ID"].astype(int) == selected_id
            ].iloc[0]
            ...
            new_time = st.selectbox(
                ...
                AVAILABLE_TIMES,
                index=time_index,
                key=f"single_new_time_{selected_id}",
            )
